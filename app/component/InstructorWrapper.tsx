    ...
    const onSubmit: SubmitHandler<UserForm> = async (datas: UserForm) => {
        if (editMode) {
            ...
            if (selectedImageFile) {
                try {
                    const imageData = await readFileAsDataURL(selectedImageFile);
                    const secondresponse = await axios.put(`${process.env.NEXT_PUBLIC_DOMAIN}/api/course`, { ...editedCourse!, image: imageData, ...datas });
                    if (secondresponse.status === 200) {
                        const updatedUser: Course = { ...editedCourse!, url: secondresponse.data.secure_url, ...datas };
                ...
                try {
                    const secondresponse = await axios.put(`${process.env.NEXT_PUBLIC_DOMAIN}/api/course`, { ...editedCourse,image:"", ...datas });
                    if (secondresponse.status === 200) {
            ...
            try {
                const imageData = await readFileAsDataURL(selectedImageFile!);
                const secondresponse = await axios.post(`${process.env.NEXT_PUBLIC_DOMAIN}/api/course`, { ...datas, image: imageData, userId: data?._id });
                if (secondresponse.status === 200) {
                    console.log(secondresponse)
                    const newUser: Course = { ...datas, url: secondresponse.data.secure_url, _id: userState.courses.length + 1 };
        ...
        <div>
            <div>
                ...
                <Modal clear={cleardatas} isOpen={isModalOpen} toggleModal={toggleModal}>
                    {deletemodal === false ? (
                        <>
                            <h2 className="text-center font-bold text-xl">{!editMode ? "Add Course" : "Update Course"}</h2>
                            <form
                            ...
                            >
                                ...
                                <Button disabled={!loading} loading={loading} plus={plus && !editMode}>
                                    {(!editMode && loading)
                                        ? "Adding Course"
                                        : (editMode && loading)
                                            ? "Updating Course"
